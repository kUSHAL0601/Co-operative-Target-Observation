...
class Main:
	...
	def initialize_param(self,no_targets,no_observers,sensor_range_targets,sensor_range_observers):
		...
		observers=[]
		for i in range(no_targets):
			targets.append(T(R()*150,R()*150,self.target_speed[RI(0,5)],R()*360,self.sensor_range_targets))
		for i in range(no_observers):
			observers.append(O(R()*150,R()*150,self.observer_speed,self.sensor_range_observers))
		return (no_targets,no_observers,targets,observers)
