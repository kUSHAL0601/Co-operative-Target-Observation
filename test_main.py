...
def initialize(no_targets,no_observers,sensor_range_targets,sensor_range_observers):
	targets=[]
	observers=[]
	for i in range(no_targets):
		targets.append(T(R()*150,R()*150,target_speed[RI(0,5)],R()*360,sensor_range_targets))
	for i in range(no_observers):
		observers.append(O(R()*150,R()*150,observer_speed,sensor_range_observers))
		...
		for obsver in no_observers_arr:
			for sns_rng_t in sensor_range:
				for sns_rng_o in sensor_range:
					(no_targets,no_observers,targets,observers)=initialize(trgt,obsver,sns_rng_t,sns_rng_o)
